...
template <typename T> T to_integer(const string& s) {
	const string str(s);
	try {
...
string Description::generateApplicationSdp(const string& eol) const {
	std::ostringstream sdp;
...
string Description::Entry::generateSdp(const string& eol, const string& addr, uint16_t port) const {
	std::ostringstream sdp;
	// RFC 3264: Existing media streams are removed by creating a new SDP with the port number for
...
string Description::Entry::generateSdpLines(const string& eol) const {
	std::ostringstream sdp;
	sdp << "a=mid:" << mMid << eol;
...
int Description::Entry::ExtMap::parseId(const string& description) {
...
Description::Entry::ExtMap::ExtMap(const string& description) { setDescription(description); }

void Description::Entry::ExtMap::setDescription(const string& description) {
...
uint16_t Description::Application::sctpPort() const { return mSctpPort; }

size_t Description::Application::maxMessageSize() const { return mMaxMessageSize; }

string Description::Application::generateSdpLines(const string& eol) const {
	std::ostringstream sdp;
