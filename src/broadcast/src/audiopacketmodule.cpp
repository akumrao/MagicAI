...
AudioPacketModule::~AudioPacketModule()
{
    if (_processThread) { _processThread->Stop(); }
}

rtc::scoped_refptr<AudioPacketModule> AudioPacketModule::Create()
{
    rtc::scoped_refptr<AudioPacketModule> capture_module(new rtc::RefCountedObject<AudioPacketModule>());
    if (!capture_module->Initialize()) { return nullptr; }
    return capture_module;
}
...
void AudioPacketModule::processFrameP()
{
...

    {
        ...
        if (_playing) { receiveFrameP(); }
        if (_recording) { sendFrameP(); }
    }
    ...
    if (last_time) std::cout << " wait time " << current_time - last_time << std::endl << std::flush;
...
void AudioPacketModule::sendFrameP()
{
    assert(_processThread->IsCurrent());
    rtc::CritScope cs(&_critCallback);
    LDebug("Send audio", _recording) if (!_audioCallback) { return; }
    ...
    //           test[159] = nCount;
    //
    //          //  std::cout <<  nCount << " first " << (int) test[0]  <<  " "  <<  (int)test[159]<< std::endl
    //          << std::flush;

    //           ++nCount;

    LTrace("Send audio") if (
...
int32_t AudioPacketModule::StopPlayout()
{
    LDebug("Stop playout") bool start = false;
    {
        rtc::CritScope cs(&_crit);
        _playing = false;
...
int32_t AudioPacketModule::StopRecording()
{
    LDebug("Stop recording") bool start = false;
    {
        rtc::CritScope cs(&_crit);
        _recording = false;
        start = shouldStartProcessing();
